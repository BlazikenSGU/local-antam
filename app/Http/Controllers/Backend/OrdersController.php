...
class OrdersController extends BaseBackendController
{
    ...
    {
        $orderAll = Orders::orderBy('id', 'desc')->get();
        $orders = Orders::orderBy('id', 'desc')->paginate(50);
        $count_status = array_fill(1, 8, 0);

        foreach ($orderAll as $item) {
            for ($i = 1; $i <= 8; $i++) {
                if (in_array($item->statusName, constant("App\\Models\\Orders::STATUS_GHN_$i"))) {
                    $count_status[$i]++;
                    break;
                }
            }
        ...
        $this->_data['count_status'] = $count_status;
    ...
    public function search(Request $request)
    {
        ...
        foreach ($orders as $item) {
            $status = $statusName->firstWhere('key', $item->statusName); // Tìm status tương ứng
            if ($status) {
        ...
        foreach ($orderAll as $item) {
            ...
            if (in_array($status, Orders::STATUS_GHN_1)) {
                ...
                $count_status[2]++;
            } elseif (in_array($status, Orders::STATUS_GHN_3)) {
                $count_status[3]++;
        ...
        if (!empty($request)) {
            ...
            if ($status == 1) {
                ...
                $status =  Orders::STATUS_GHN_2;
            } elseif ($status == 3) {
                $status =  Orders::STATUS_GHN_3;
            } elseif ($status == 4) {
                $status =  Orders::STATUS_GHN_4;
            } elseif ($status == 5) {
                $status =  Orders::STATUS_GHN_5;
            } elseif ($status == 6) {
                $status =  Orders::STATUS_GHN_6;
            } elseif ($status == 7) {
                $status =  Orders::STATUS_GHN_7;
            } elseif ($status == 8) {
                $status = Orders::STATUS_GHN_8;
            }

            if (!empty($status)) {
                $orders = Orders::whereIn('statusName', $status)
                    ->orderBy('created_at', 'desc')
                    ->paginate(20)
                    ->appends($request->query());
            } elseif ($keyword) {
                $orders = Orders::where('to_name', 'LIKE', '%' . $keyword . '%')
                    ->orWhere('to_phone', 'LIKE', '%' . $keyword . '%')
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($cod_amount != $requestcod_amount) {
                $params = [
                    ...
                    'cod_amount' => $requestcod_amount + $phiAdmin,
                ];
            ...
            if ($data['r'] == 1) {
                ...
                //                    ->first()->shopId;
                $product_type_cost =   \App\Models\SettingFee::get_by_where($order->user_id, $order->product_type)->cost ?? 0;
                //
                //
    ...
    public function update1(Request $request)
    {
        ...
        $order->payment_fee  = $payment_method == 2 ? $phiAdmin : 0;
        $order->caGiaohang = $request->get('caGiaohang', $order->caGiaohang);
        $order->to_phone = $request->get('to_phone', $order->to_phone);
        $order->to_name = $request->get('to_username', $order->to_name);
        $order->to_address = $request->get('to_address', $order->to_address);
        $order->to_province = $request->get('to_province', $order->to_province);
        $order->to_province_name = $request->get('province_name', $order->to_province_name);
        $order->to_district = $request->get('to_district', $order->to_district);
        $order->to_district_name = $request->get('district_name', $order->to_district_name);
        $order->to_ward = $request->get('to_ward', $order->to_ward);
        $order->to_ward_name = $request->get('ward_name', $order->to_ward_name);
        $order->weight = $request->get('weight', $order->weight);
        $order->length = $request->get('length', $order->length);
        $order->width = $request->get('width', $order->width);
        $order->height = $request->get('height', $order->height);
        $order->required_note = $request->get('required_note', $order->required_note);
        $order->order_code = $request->get('order_code', $order->order_code);
        $order->note = $request->get('note', $order->note);
        $order->payment_method = $request->get('payment_method', $order->payment_method);
        $order->cod_amount = $request->get('cod_amount', $order->cod_amount);
        $order->cod_failed_amount = $request->get('cod_failed_amount', $order->cod_failed_amount);
        $order->product_type = $request->get('product_type', $order->product_type);
        $order->insurance_value = $request->get('insurance_value', $order->insurance_value);
        $order->order_code_custom = $request->get('order_code_custom', $order->order_code_custom);
        $order->return_phone = $request->get('return_phone', $order->return_phone);
        $order->return_address = $request->get('return_address', $order->return_address);
        $order->product_type_cost = $request->get('product_type_cost', $order->product_type_cost);
        //$order->update($params);
        //$order->save();
        $totalWight = 0;
        $weight = $order->weight;
        ...
        if (!empty($order->to_district) and !empty($order->to_ward)) {
            $fee = [
                ...
                "weight"                => intval($totalWight),
                "width"                 => 0,
                "insurance_value"       => intval(str_replace(['.', ','], '', $request->get('insurance_value', $order->insurance_value))),
            ];
            $getFee = GHN::GetFee($fee);
            $order->total_fee = $getFee['data']['service_fee'] + $order->product_type_cost;
            $order->insurance_fee = $getFee['data']['insurance_fee'];
        ...
        return \Response::json(['e' => $getFee, 'r' => $order, 't' => $totalWight, 'j' => $fee]);
    }
    ...
    public function getPhone(Request $request)
    {
        ...
        $user_id = Auth::guard('backend')->user()->id;
        //$stores = Address::where('user_id', Auth()->guard('backend')->user()->id)->pluck('phone')->toArray();

        $order = Orders::find($order_id);
        $shop_phonex = $order->phone;
        //$d = Orders::where('to_phone', $phone)->where('phone',$shop_phonex)->whereNotNull('to_name')->where('statusName', '!=' , 'cancel')->where('statusName', '!=' , 'ready_to_pick')->where('statusName', '!=' , 'create_order')->latest('created_at')->first();

        $d = Orders::where('to_phone', $phone)->where('user_id', $user_id)->whereNotNull('to_name')->whereNotIn('statusName', ['cancel', 'ready_to_pick', 'create_order'])->latest('created_at')->first();

        return \Response::json(['e' => 0, 'r' => $d, 'user_id' => $user_id]);
    }

    public function getRPhone(Request $request)
    {
        //$objModel = new Orders();
        $rphone =  $request->get('rphone');
        $shop_phone = Auth()->guard('backend')->user()->phone;
        $order_id =  $request->get('orderid');
        $order = Orders::find($order_id);
        $user_id = Auth::guard('backend')->user()->id;
        $shop_phonex = $order->phone;
        $d = Orders::where('return_phone', $rphone)->where('user_id', $user_id)->whereNotIn('statusName', ['cancel', 'ready_to_pick', 'create_order'])->latest('created_at')->first();
        return \Response::json(['e' => 0, 'r' =>  $d]);
    }
}

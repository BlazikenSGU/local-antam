...
//                                    $errors[] = $index + 1; // Vì index bắt đầu từ 0, nên cộng thêm 1 để đếm từ 1
    ...
    public function update(Request $request)
    {
        ...
        if (!empty($order->to_district) and !empty($order->to_ward)) {
            ...
            $getFee = GHN::GetFee($fee);
            $order->total_fee = 10;//$getFee['data']['service_fee'] + $order->product_type_cost;
            $order->insurance_fee = $getFee['data']['insurance_fee'];
            $order->main_service = $getFee['data']['service_fee'] + $order->product_type_cost + $getFee['data']['insurance_fee'];
            $order->save();
        }
...
//
    ...
    public function getPhone(Request $request)
    {
        ...
        //$d = Orders::where('to_phone', $phone)->where('phone',$shop_phonex)->whereNotNull('to_name')->latest('created_at')->get();
        return \Response::json(['e' => 0, 'r' => 1, 'phone'=>$phone, 'shop_phonex'=>$shop_phonex]);
    }
    ...
    public function getRPhone(Request $request)
    {
        ...
        $order = Orders::find($order_id);
        $shop_phonex = $order->phone; 
        $d = Orders::where('return_phone', $rphone)->where('phone',$shop_phonex)->where('statusName', '!=' , 'cancel')->where('statusName', '!=' , 'ready_to_pick')->where('statusName', '!=' , 'create_order')->latest('created_at')->first();
